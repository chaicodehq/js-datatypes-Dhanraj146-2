...
export function processRailwayPNR(pnrData) {
  if (
    ...
    !Object.hasOwn(pnrData, "train") ||
    typeof pnrData.train !== "object" ||
    pnrData.train === null ||
    Array.isArray(pnrData.train) ||
  ...
  const passengers = pnrData.passengers.map((passenger) => {
    ...
    if (currentStatus.startsWith("B") || currentStatus.startsWith("S"))
      statusLabel = "CONFIRMED";
    else if (currentStatus.startsWith("WL")) statusLabel = "WAITING";
    else if (currentStatus === "CAN") statusLabel = "CANCELLED";
  ...
  const confirmed = passengers.filter(
    (passenger) => passenger.isConfirmed,
  ).length;
  ...
  const allConfirmed = passengers.every((passenger) => passenger.isConfirmed);
  ...
  const chartPrepared =
    totalPassengers - cancelled === confirmed ? true : false;
  return {
    pnrFormatted,
    trainInfo,
    passengers,
